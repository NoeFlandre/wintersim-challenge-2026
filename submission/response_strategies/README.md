...
`assign_associated_bookings`, and `adjust_bookings_before_cargo_handling` to
the organizer fallback (each returns `None`).

`create_alternative_service_routes` is also a no-op when no disruption is
active (returning `None` so the fallback can clean up and restore vessels to
their original routes). While at least one disruption is active, however, it
returns `False` to tell the caller "handled, do not run the organizer
alternative-route fallback". The active call makes **no mutation whatsoever**
on the context, routes, vessels, legs, or the supplied vessel sentinel.

The exact rationale, hypothesis, and acceptance rules are documented in
`docs/experiments/round0-alternative-route-suppression-v1.md`.
