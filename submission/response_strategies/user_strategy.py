...
def _segment_for_index(route: Any, sequence_index: Any) -> Any | None:
    ...
    matches = [
        ...
        if getattr(segment, "sequence_index", None) == sequence_index
    ]
...
def _booking_edge(
    ...
    booking: Any,
    destination: Any,
) -> tuple[_Edge, Any] | None:
    route = getattr(booking, "service_route", None)
    if route is None or getattr(vessel, "assigned_service_route", None) is not route:
        return None
    current_leg = getattr(current_segment, "associated_leg", None)
    current_port = getattr(current_leg, "arrival_port", None)
    if current_port is None:
        return None
    segments = getattr(route, "segments", None)
    if not isinstance(segments, (list, tuple)) or len(segments) < 2:
        return None
    ordered_segments = sorted(segments, key=lambda segment: getattr(segment, "sequence_index", -1))
    if len({id(segment) for segment in ordered_segments}) != len(ordered_segments):
        return None
    current_index = next(
        (index for index, segment in enumerate(ordered_segments) if segment is current_segment),
        None,
    )
    if current_index is None:
        return None
    departure_segment = _segment_for_index(route, getattr(booking, "departure_segment_index", None))
    arrival_segment = _segment_for_index(route, getattr(booking, "arrival_segment_index", None))
    if departure_segment is None or arrival_segment is None:
        return None
    next_segment = ordered_segments[(current_index + 1) % len(ordered_segments)]
    if next_segment is not departure_segment:
        return None
    route_data = _route_data(route)
    if route_data is None:
        return None
    matches = [
        ...
        if (
            edge.departure is current_port
            and edge.arrival is destination
            and edge.legs
            and edge.legs[0] is getattr(departure_segment, "associated_leg", None)
            and edge.legs[-1] is getattr(arrival_segment, "associated_leg", None)
        )
    ]
    return (matches[0], current_port) if len(matches) == 1 else None


def _same_edge(left: _Edge, right: _Edge) -> bool:
    return (
        left.route is right.route
        and left.departure is right.departure
        and left.arrival is right.arrival
        and left.legs == right.legs
    )


def _should_hold_current_direct(context: Any, now: Any, vessel: Any) -> bool:
    if not isinstance(now, dt.datetime):
        return False
    carried = getattr(vessel, "carried_shipments", None)
    current_segment = getattr(vessel, "current_segment", None)
    if not isinstance(carried, list) or not carried or current_segment is None:
        return False
    ...
    for shipment in carried:
        ...
        current_bookings = [
            ...
            if getattr(booking, "sequence_index", None) == current_index
        ]
        if len(current_bookings) != 1:
            return False
        booking = current_bookings[0]
        booking_edge = _booking_edge(vessel, current_segment, booking, destination)
        if booking_edge is None:
            return False
        edge, current_port = booking_edge
        recovery = _edge_constraint_recovery(edge, state)
        if recovery is None:
            continue
        ...
        if (
            len(bookings) != 1
            or getattr(booking.service_route, "source_service_route", None) is not None
        ):
            return False
        nominal_path = _shortest_path(context, current_port, destination, graphs[0])
        safe_path = _shortest_path(context, current_port, destination, graphs[1])
        if (
            nominal_path is None
            or safe_path is None
            or len(nominal_path) != 1
            or len(safe_path) < 2
        ):
            return False
        nominal_edge = nominal_path[0]
        if not _same_edge(nominal_edge, edge):
            return False
        ...
        if not hold_hours < detour_hours:
            return False
        qualifying += 1
    return affected > 0 and qualifying == affected
...
class UserStrategy:
    ...
    def adjust_bookings_before_cargo_handling(context: Any, now: Any, vessel: Any) -> Any:
        """Hold carried direct cargo when waiting beats a multi-transfer detour."""
        try:
            return False if _should_hold_current_direct(context, now, vessel) else None
        except _DATA_ERRORS:
            return None
