...
## Current strategy: safe-departure opportunity gate v4
...
recover and deliver sooner than that detour. The remaining recovery wait must
also be no longer than one live headway of the detour's first safe service, so
the hold does not skip multiple safe departure opportunities.
...
also delegates without mutation. The v4 refinement has no performance result
until its pre-registered full experiment finishes; the preceding accepted v3
control remains `19.084638612143134` over 72 periods.
