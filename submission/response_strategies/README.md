...
## Current strategy: multi-TEU missed-connection buffer experiment
...
(at least three service boardings), and either the direct service is estimated
to recover and deliver sooner than that detour or the shipment is an integer
multi-TEU load and remains faster after adding one minimum safe-route headway
for a possible capacity-driven missed connection. Exact equality delegates.
...
full-precision comparison, and otherwise delegates. The buffer is derived from
the live safe routes and is not a configured threshold. Missing or ambiguous
data also delegates without mutation. No performance result is claimed until
the pre-registered full experiment finishes.
