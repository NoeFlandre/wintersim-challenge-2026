...
(at least three service boardings), uses each deployed service route at most
once, and the direct service is estimated to recover and deliver sooner than
that detour. A safe path that returns to a service route already used earlier
is delegated because it is a cyclic detour rather than a clean sequence of
distinct services.
