...
## Current strategy: v21 multi-TEU mixed recovery hold
...
recover and deliver sooner than that detour. The v21 extension preserves those
holds and adds exactly one narrower case: a finite, non-boolean `teu_size`
strictly greater than one, a one-change safe route, and both a matching
congested leg and closed arrival port on the nominal edge. Exact one-TEU,
missing, malformed, non-finite, and non-positive cargo sizes delegate the
one-change case.
