...
is installed for a new shipment. Every decision is derived from the supplied
runtime objects, is deterministic, and delegates on uncertainty.
"""
...
def _fewer_transfer_equal_path(
    ...
    """Find an exact-distance safe path with fewer route changes."""
...
def _install_equal_tie_path(shipment: Any, path: tuple[_Edge, ...]) -> bool | None:
    """Create the candidate booking chain and roll back if appending fails."""
...
class UserStrategy:
    ...
    def assign_associated_bookings(context: Any, now: Any, shipment: Any) -> Any:
        """Preserve v3 holds and improve exact-distance safe-path ties."""
        try:
