...
**Status: DESIGN — implementation and run pending**  
**Branch:** `main`  
...
than waiting for the direct service to recover. The control's read-only audit
found 254 such observations. A structural timing audit found that 98 occur
while the remaining recovery wait is at least one safe-route headway, during
the early part of the closure windows; 156 occur later, when recovery is
already within one headway.
...
model advancement, no Output write, and no participant mutation. The expected
structural result is 254 control holds, 156 candidate holds, 98 control-only
delegations, and an annual-TEU removal proxy of 219,695; these values must be
recomputed and recorded rather than trusted from this design.
