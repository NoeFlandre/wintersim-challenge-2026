...
def _integer_day_midpoints(context: Any) -> tuple[dt.datetime, ...]:
    times: list[dt.datetime] = []
    for plan in context.disruption_plans:
        start_days = getattr(plan, "start_offset_days", None)
        duration_days = getattr(plan, "duration_days", None)
        if (
            ...
            first_day = math.ceil(start_days)
            last_day = math.floor(start_days + duration_days - 1e-12)
            times.extend(
...
def test_real_round1_pure_leg_hold_is_candidate_only_and_mutation_free() -> None:
    source = _bootstrap_or_skip()
    _prepare_imports(source)

    import main  # type: ignore[import-not-found]  # noqa: F401, PLC0415
    import scenario_builders  # type: ignore[import-not-found]  # noqa: PLC0415
    from maritime_data_context.shipment import (  # type: ignore[import-not-found]  # noqa: F401, PLC0415
        Shipment,
    )
    from response_strategies.default_strategy import (  # type: ignore[import-not-found]  # noqa: F401, PLC0415
        DefaultStrategy,
    )

    participant = _load_participant_module()
    template = scenario_builders.create_with_disruption()
    times = _integer_day_midpoints(template)
    found = False
    for now in times:
        context = scenario_builders.create_with_disruption()
        DefaultStrategy.create_alternative_service_routes(context, now)
        for index, demand in enumerate(context.demands):
            ...
            if route_changes < 2:
                continue
            nominal_edge = nominal[0]
            leg_ids = {id(leg) for leg in nominal_edge.legs}
            arrival_names = {
                getattr(port, "name", "").casefold()
                for port in (*nominal_edge.intermediate_ports, nominal_edge.arrival)
            }
            kinds = {
                constraint.kind
    ...
    assert found, "no structurally derived pure-leg v3 activation was found"
