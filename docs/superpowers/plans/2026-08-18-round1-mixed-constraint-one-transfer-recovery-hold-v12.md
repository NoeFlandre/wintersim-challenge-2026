...
## Phase 1: read-only audit and design freeze
...
5. Run focused GREEN tests, then all formatter, linter, Ty, mypy, coverage, and
   integration gates. Commit the completed preflight record and ignored
   non-overwriting manifest only when all gates pass.
...
and no live process. Freeze the manifest at the reviewed HEAD and stop. Do not
launch the full run until a senior review explicitly authorizes it.
