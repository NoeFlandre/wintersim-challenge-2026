...
change for a port-closure-only detour with a full-headway safety margin when
recovery is already within one safe-route headway.
...
def _should_hold(context: Any, now: Any, shipment: Any) -> bool:
    ...
    return max_headway is not None and wait_hours < max_headway and margin > max_headway
