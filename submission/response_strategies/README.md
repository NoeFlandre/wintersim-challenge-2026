...
the fastest safe route needs at least two changes between services (at least
three service boardings), and the direct service is estimated to recover and
deliver sooner than that detour. The safe-route estimate uses sailing time
plus half a service-route headway whenever it enters a different route, with
context-order tie breaking.
