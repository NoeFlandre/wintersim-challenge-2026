...
## Current strategy: buffered mixed-transfer recovery hold
...
The v15 experiment additionally permits exactly one service change only when
the disrupted direct edge matches both an active leg and active port
constraint, and the detour's estimated advantage exceeds half of the first
safe service's headway. This buffer is strict; equality and all uncertainty
