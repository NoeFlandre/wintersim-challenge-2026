...
def test_real_round1_context_contains_multi_teu_buffer_activation() -> None:
    source = _bootstrap_or_skip()
    _prepare_imports(source)
    ...
    participant = _load_participant_module()
    times = _candidate_times(scenario_builders.create_with_disruption())
    activation: tuple[Any, ...] | None = None
    ...
    for now in times:
        ...
        for index, demand in enumerate(context.demands):
            ...
            if decisions == [None, False]:
                activation = (context, now, demand)
                break
        if activation is not None:
            break

    assert activation is not None, (
        "v20 is dormant: no real-context observation delegates one TEU while "
        "holding the same two-TEU demand inside the missed-connection buffer"
    )

    context, now, demand = activation
    state = participant._active_state(context, now)
    assert state is not None
    graphs = participant._graphs(context, state)
    assert graphs is not None
    ...
    assert nominal_path is not None and len(nominal_path) == 1
    assert safe_path is not None
    recovery = participant._edge_constraint_recovery(nominal_path[0], state)
    nominal_hours = participant._path_service_hours(nominal_path)
    detour_hours = participant._path_service_hours(safe_path)
    buffer_hours = participant._minimum_path_headway_hours(safe_path)
    assert recovery is not None
    assert nominal_hours is not None
    assert detour_hours is not None
    assert buffer_hours is not None
    hold_hours = max(0.0, (recovery - now).total_seconds() / 3600.0) + nominal_hours
    assert hold_hours >= detour_hours
    assert hold_hours < detour_hours + buffer_hours
